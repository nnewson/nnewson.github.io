...
A Catch2 `SECTION` is a named branch inside a `TEST_CASE`. Catch2 reruns the
enclosing test case's setup for each reachable section path, which is useful
when several invalid inputs share setup. CTest still reports the surrounding
`TEST_CASE` as one of the 24 discovered entries rather than reporting each
section separately.
...
`.cpp` files only. Catch2 expresses test cases and assertions through macros
that generate framework plumbing around the code written in each test. Applying
the existing production configuration to those translation units also surfaces
diagnostics through that generated expansion, so the tests need a test-specific
configuration or narrowly scoped suppressions. Release 0.7 has not introduced
that configuration, and the CI comment records the limitation instead of
weakening production analysis to accommodate it.
...
- clang-tidy remains strict over production sources while Catch2-heavy tests
  remain excluded pending a test-specific analysis configuration for the
  framework code generated by its macros;
...
The build requests Catch2 version 3; a separately installed Catch2 version 2
...
clang-tidy requires a test-specific configuration or narrowly scoped
suppressions for the framework plumbing generated by Catch2's macros, without
weakening analysis of the production sources.
