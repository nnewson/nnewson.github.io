...
Changing settings independently until AnimatedCube looks plausible could
produce the right image by coincidence. Treating them as one contract makes
the result explainable: the shader produces Vulkan-compatible clip
coordinates, culling interprets their winding consistently, and the depth test
compares values in the range generated by the projection.

## Build one fixed view-projection matrix

The tutorial camera is fixed. It sits four units along positive Z,
...
```text
swapchain extent ──┬──> projection aspect ratio
                   └──> DepthBuffer ──> depth format ──┐
                                                       ├──> graphics Pipeline
...
The image uses optimal tiling, so the renderer requires
`eDepthStencilAttachment` in the format's `optimalTilingFeatures`.
...
then its contents are forgotten before the next clear.
...
Rotation makes a winding error obvious: the wrong faces remain visible, or the
cube's exterior disappears as it turns. A single triangle viewed from one fixed
side does not test both orientations.

This is still a narrow opaque policy. A negative-determinant
...
These responsibilities sit at different test levels. Projection
